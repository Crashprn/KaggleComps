...
def convert_dayofyear_to_month_day(df, col):
    days_in_months = [calendar.monthrange(2023, month)[1] for month in range(1, 13)]
    cumulative_days = np.cumsum(np.array(days_in_months))

    month = np.searchsorted(cumulative_days, df[col]) + 1

    df['month'] = month
    df['day_of_month'] = df[col] - np.concatenate(([0], cumulative_days[:-1]))[month - 1]
